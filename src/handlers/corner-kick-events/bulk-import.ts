...
import { bulkImportCornerKickEventsSchema } from '../../validation/corner-kick-event';
...
export const handler = withHandler(async (event) => {
  ...
  const { entries } = bulkImportCornerKickEventsSchema.parse(parseJsonBody(event));
  ...
  const rows = await prisma.$transaction(
    entries.map((entry) => prisma.cornerKickEvent.create({ data: entry })),
  );
